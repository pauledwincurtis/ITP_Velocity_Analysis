...
function [depth_of_MLD, salinity_of_MLD, time_of_MLD, actual_time_MLD, final_profile_list] = find_MLD_pdt(path, system, threshold, PRESSURE_RANGE, direction)
    ...
    for i = profile_list
        ...
        for j = 1:length(pdens)
            if pdens(j) - SD >= threshold
                ...
                salinity_of_MLD = [salinity_of_MLD, profiles(i).salinity(j-1)];
