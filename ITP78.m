...
% 
% The following code is a simplified version of the main ITP78 code to
...
common_grid_TH = 130:-0.25:10; % common grid (CG) for THERMOHALINE interpolation (this should ensure monotonicity of the P vector)
common_grid_V = 100:(-1/7):10; % common grid (CG) for VELOCITY interpolation (this is higher fidelity than TH)
...
MLD_CT_minus_CTF_minus_X_array = []; % CT - CTF X metres above the base of the mixed layer
...
SAL_hovmoller_rel2maxN2 = []; % Conservaitve temperature
...
SAL_hovmoller_rel2maxN2_deep_field = [];
...
% now the loop...
for i = 1:1300 % total number of profiles (1694 total)

    dont_take = [75 253 254 260 659 700 931 1006 1247 1319 1506 1517 1626 1627 1629 1651];
    ...
    fmt = ' ';
    fname = sprintf(fmt, i);
    profiles_ITP78_V = open(fname);

    % PART TWO A - Load in the thermohaline variables
    pressure = profiles_ITP78_V.pr_filt; % best estimate of pressure (TH)
    salinity = profiles_ITP78_V.sa_adj; % best estimate of salinity (TH)
    temperature = profiles_ITP78_V.te_cor; % best estimate of temperature (TH)
    latitude = profiles_ITP78_V.latitude; % latitude

    % Part TWO B - Load in the velocity variables 
    pressure_V = profiles_ITP78_V.v_prfilt; % pressure for the velocity sensor (V)
    zonal_V = profiles_ITP78_V.v_east/100; % zonal velocity in m/s (V)
    meridional_V = profiles_ITP78_V.v_north/100; % meridional velocity in m/s (V)
    ...
    if (mean(pressure) <= 10) || min(pressure) >= 10 || mean(pressure_V) <= 10 || all(isnan(pressure))
    ...
    start_date = profiles_ITP78_V.psdate; % "mm/dd/yy"
    start_time = profiles_ITP78_V.pstart; % "hh:mm:ss"
    ...
    longitude_array = [longitude_array, profiles_ITP78_V.longitude]; % longitude
    latitude_array = [latitude_array, profiles_ITP78_V.latitude]; % latitude
    ...
    mod_u_10dbar = mod_u_CG_int(find(common_grid_V == 10));
    mod_u_20dbar = mod_u_CG_int(find(common_grid_V == 20));
    mod_u_30dbar = mod_u_CG_int(find(common_grid_V == 30));
    ...
    for j = length(potential_density_CG):-1:1 % the last measurment is the shallowest
        if potential_density_CG(j) - surface_density >= threshold_MLD
            ...
            potential_density_of_MLD_minus_X = potential_density_CG(j+1+20); % Takes the potential density 
            CT_minus_CTF_of_MLD_minus_X = conservative_temperature_CG(j+1+12) - conservative_temperature_freezing_CG(j+1+12); % Takes the potential density 
    ...
    %
    % 3) Mixed layer depth on interpolated grid (for estimating dT/dz)
    for j = length(potential_density_CG_int):-1:1 % the last measurment is the shallowest
        if potential_density_CG_int(j) - surface_density >= threshold_MLD
            pressure_of_MLD_int = common_grid_TH(j+1); % Remember that in these profiles, the density is increasing up with decreasing index
            index_MLD_int = j + 1;
            break
        end
    end
    ...
    SAL_CG_rel2maxN2 = pchip(pressure_CG - p_max_N_2, salinity_CG, -4:0.1:4); % puts CTF relative to max N^2
    ...
    SAL_CG_rel2maxN2_deep_field = pchip(pressure_CG - p_max_N_2, salinity_CG, -5:0.2:80); % puts CTF relative to max N^2
    ...
    zonal_V_CG_int_rel2maxN2_deep_field = pchip(common_grid_V - p_max_N_2, zonal_V_CG_int, -15:0.1:80);
    meridional_V_CG_int_rel2maxN2_deep_field = pchip(common_grid_V - p_max_N_2, meridional_V_CG_int, -15:0.1:80);
    %
    ...
    MLD_int_array = [MLD_int_array, pressure_of_MLD_int]; % appends the mixed layer pressure on interpolated grid
    MLD_int_index_array = [MLD_int_index_array, index_MLD_int]; % appends " " index
    ...
    MLD_CT_minus_CTF_minus_X_array = [MLD_CT_minus_CTF_minus_X_array, CT_minus_CTF_of_MLD_minus_X];
    ...
    SAL_hovmoller_rel2maxN2 = [SAL_hovmoller_rel2maxN2, SAL_CG_rel2maxN2.']; % appends CT - CTF relative to max N2
    ...
    SAL_hovmoller_rel2maxN2_deep_field = [SAL_hovmoller_rel2maxN2_deep_field, SAL_CG_rel2maxN2_deep_field.']; 
