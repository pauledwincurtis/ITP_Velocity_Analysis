...
% 
% The following code is a simplified version of the main ITP35 code to
...
common_grid_TH = 130:-0.25:7; % common grid (CG) for THERMOHALINE interpolation (this should ensure monotonicity of the P vector)
...
for i = 1:1357 % total number of profiles (1079/1357)
    ...
    pressure = profiles_ITP35_V.pr_filt; % best estimate of pressure (TH)
    salinity = profiles_ITP35_V.sa_adj; % best estimate of salinity (TH)
    temperature = profiles_ITP35_V.te_cor; % best estimate of temperature (TH)
    latitude = profiles_ITP35_V.latitude; % latitude

    % Part TWO B - Load in the velocity variables 
    pressure_V = profiles_ITP35_V.v_prfilt; % pressure for the velocity sensor (V)
    zonal_V = profiles_ITP35_V.v_east/100; % zonal velocity in m/s (V)
    meridional_V = profiles_ITP35_V.v_north/100; % meridional velocity in m/s (V)
    ...
    start_date = profiles_ITP35_V.psdate; % "mm/dd/yy"
    start_time = profiles_ITP35_V.pstart; % "hh:mm:ss"
    ...
    longitude_array = [longitude_array, profiles_ITP35_V.longitude]; % longitude
    latitude_array = [latitude_array, profiles_ITP35_V.latitude]; % latitude
